...
function Cell({ col, value, onChange, color, readOnly }) {
  const base = {
    width: '100%', background: readOnly ? '#040a12' : '#060d18', color: '#e2e8f0',
    border: `1.5px solid ${readOnly ? '#0f172a' : '#1e293b'}`,
    borderRadius: 6, padding: '7px 10px',
    fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: 12, outline: 'none',
    boxSizing: 'border-box', transition: 'border-color .15s',
    cursor: readOnly ? 'default' : 'text',
  }
  const focus = (e) => { if (!readOnly) e.target.style.borderColor = color }
  const blur  = (e) => { e.target.style.borderColor = readOnly ? '#0f172a' : '#1e293b' }
  ...
  if (col.type === 'textarea') {
    return (
      <textarea value={value || ''} onChange={e => !readOnly && onChange(e.target.value)}
        rows={2} placeholder={readOnly ? '' : col.label} readOnly={readOnly}
  ...
  return (
    <input
      ...
      placeholder={readOnly ? '' : col.label} readOnly={readOnly}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 500 }}>
            <thead>
              <tr style={{ background: '#0a1520' }}>
                <th style={thStyle}>#</th>
                {inlineCols.map(c => <th key={c.key} style={thStyle}>{c.label}</th>)}
              ...
              {rows.map((row, idx) => (
                <>
                  <tr key={row._id || idx} style={{
                    ...
                    <td style={{ ...tdStyle, color: '#475569', fontFamily: 'monospace', width: 36 }}>{idx + 1}</td>
                    {inlineCols.map(c => (
                  ...
                  {hasExtra && expandedRow === idx && (
                    <tr key={`${idx}-extra`} style={{ background: '#0a1929', borderBottom: '1px solid #1e293b' }}>
                      <td />
                      <td colSpan={inlineCols.length + (readOnly ? 1 : 2)} style={{ padding: '12px 16px' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px,1fr))', gap: 10 }}>
                          {extraCols.map(c => (
                            <div key={c.key}>
                              <div style={{ fontSize: 10, color: '#475569', marginBottom: 4, fontFamily: 'monospace', letterSpacing: 0.5 }}>
                                {c.label.toUpperCase()}
                              </div>
                              <Cell col={c} value={row[c.key]} color={color} readOnly={readOnly}
                                onChange={val => updateCell(idx, c.key, val)} />
      ...
      {rows.length === 0 && (
        <div style={{
          ...
          textAlign: 'center', color: '#334155', fontFamily: "'Plus Jakarta Sans', sans-serif",
        }}>
        ...
        <div>
          ...
          {(response.documents || []).map(doc => (
            <div key={doc.id} style={{
              background: '#0a1929', border: '1px solid #1e293b', borderRadius: 8, padding: '8px 14px',
...
const actionBtn = (bg, fg) => ({
  background: bg, border: 'none', color: fg, borderRadius: 4,
  padding: '4px 7px', fontSize: 12, cursor: 'pointer', marginLeft: 4,
})
